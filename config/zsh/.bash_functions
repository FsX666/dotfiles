...
function rsync_send_file() {
        if [[ $1 && $2 ]]
        then
                rsync -av -rsh='ssh' -av --progress --partial $1 ${LDAP_USER}@$2:$1
        else
...
function rsync_get_file() {
        if [[ $1 && $2 ]]
        then
                rsync -av -rsh='ssh' -av --progress --partial ${LDAP_USER}@$2:$1 $1
        else
                echo -n "help: \$1: file to get and \$2: source server"                                                                                                                                                                      
        fi
}

function kill_anydesk() {
	LIST=$(ps faux | grep "anydesk" | grep -v "color" | /usr/bin/awk -F " " '{print $2}')
    for each in $LIST; do echo $each|xargs -i kill -9 {}; done
#|xargs -i kill -9 {}
}
...
if [ $? -ne 0 ];then
        ...
        . $SSH_AGENT_CONF > /dev/null
#        ssh-add
		ssh-add $(grep -slR "PRIVATE" ~/.ssh/id_*)
fi
